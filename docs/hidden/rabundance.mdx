...
import {spend, revenue, source_proc, sources_revenue_total, package_info, money, emptyRow } from "./data/data.js"
...
:::info
Lets use upper cold season value for our calculations: **50days**
<hr/>This is the number of days where we can purchase source and immediately process
:::

<CheekyTable headers={["total_source", "batch_size_kg", "batch_time_hr", "hours_to_dry", "days_to_dry"]}
 data={[{
   "total_source": source_proc.total_amount_in_kg,
   "batch_size_kg": data.source_proc.drying.batch_size_kg,
   "batch_time_hr": data.source_proc.drying.batch_time_hr,
   "hours_to_dry": source_proc.hours_to_dry,
   "days_to_dry": (source_proc.hours_to_dry/24).toFixed(1)
 }]}/>
...
<CheekyTable headers={["kw_per_hour", "total_kw", "price_per_kw", "total"]}
 data={[{
   "kw_per_hour": data.source_proc.drying.energy_per_hour_kw,
   "total_kw": source_proc.hours_to_dry * data.source_proc.drying.energy_per_hour_kw,
   "price_per_kw": data.source_proc.drying.energy_per_hour_price,
   "total": money(source_proc.hours_to_dry * data.source_proc.drying.energy_per_hour_kw * data.source_proc.drying.energy_per_hour_price)
 }]}/>
